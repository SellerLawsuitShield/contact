...
function fetchLogs(startDate, endDate, callback) {
    ...
    // ✅ Reattach country click events with a slight delay
    setTimeout(() => {
        attachCountryClickEvents();
    }, 200); // delay by 100ms to allow DOM to render
...
function attachCountryClickEvents() {
    ...
    document.querySelectorAll("td:nth-child(7)").forEach(td => {
        ...
        let cityTd = td.previousElementSibling?.previousElementSibling;
        let city = td.parentElement.querySelector("td:nth-child(5)").textContent.trim();
        ...
        if (!td.querySelector("a") && country !== "NA" && country !== "") {
            ...
            td.appendChild(countryLink); // Add the clickable country
        }
    });
